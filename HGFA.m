...
global Element GaussXW Node LE MatiA Mat AG ElNoC Node_dis
% PQNM=0采用解析解
% PQNM=1采用数值解
PQNM=1;
RIndex=1:2;
H=zeros(2,size(Element,1)*4);
G=zeros(2,size(Element,1)*4);
VF=zeros(2,1);
    for iE=1:size(ElementAi,1)
            ...
            En1=Element(ElNo,3);
            En2=Element(ElNo,4);
            En11=Element(ElNo,13);
            En21=Element(ElNo,14);
            if IntMethod==0
            ...
            elseif IntMethod==3
                ...
                if PQNM==1
                     [Hi Gi VFi]=HGFE(GaussXW,Node([En1 En2],:),LE(ElNoC),Mat(MatiA,:),AG,ElNo,np,Node_dis);
                else
%                     解析解
                    [Hi Gi VFi]=ModiHGFE(GaussXW,Node([np En1 En2],:),LE(ElNoC),Mat(MatiA,:),AG);
                end
                CIndex=[2*En11+[-1 0],2*En21+[-1 0]];
                H(RIndex,CIndex)=H(RIndex,CIndex)+Hi;
               % CIndex=[2*En11+[-1 0] 2*En21+[-1 0]];
                G(RIndex,CIndex)=G(RIndex,CIndex)+Gi;
